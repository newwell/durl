...
class ExcelFileParser {
	...
	function find_stream($dir, $item_name, $item_num = 0) {
		...
		if (($dt == 2) || ($dt == 5))
			for($i = 0;
			$i < ($dir -> getOrd($item_num * 0x80 + 0x40) + 256 * $dir -> getOrd($item_num * 0x80 + 0x41)) / 2-1;
			$i++)
...
function Read_Excel_File($ExcelFile, &$result) {
	...
	switch ($res) {
		...
		case 1: $err = "无法打开文件";
			break;
		case 2: $err = "文件太小，可能不是Excel文件";
			break;
		case 3: $err = "文件头读取错误";
			break;
		case 4: $err = "读取文件时出错";
			break;
		case 5: $err = "这不是一个Excel文件或者是Excel5.0以前版本文件";
			break;
		case 6: $err = "文件损坏";
			break;
		case 7: $err = "在文件中没有发现Excel数据";
			break;
		case 8: $err = "不支持的文件版本";
			break;
		default:
