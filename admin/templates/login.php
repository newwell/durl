...
#topbg {z-index: -1;position: absolute;left: 0;width: 100%;height: 360px;background: url(<?php echo $_TEMPLATESDIR. 'image/bg.jpg';?>) no-repeat center	top;}
...
.input {font-size: 22px;padding-left: 12px;padding-right: 0px;width: 234px;height: 30px;background: url(<?php echo $_TEMPLATESDIR. 'image/login_spacer.gif';?>);color: #383838;outline: medium none;border: none;}
.input_div {position: absolute;margin-top: 14px;width: 295px;height: 80px;background: url(<?php echo $_TEMPLATESDIR. 'image/login_input_bg.png';?>) 0 10px;}
...
.submit_bg {background: url(<?php echo $_TEMPLATESDIR. 'image/login_submit.png';?>);width: 80px;height: 42px;position: absolute;overflow: hidden;margin-top: 34px;}
...
<!--[if lt IE 6.9]>
<style type="text/css">
.input_div{background:none;}
.submit_bg{background:none;}
.input_bg_forie6{position:absolute;width:290px;height:80px;overflow:hidden;}
.input_bg_forie6 .inputbg{position:absolute;top:10px;width:286px;height:80px;filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='<?php echo $_TEMPLATESDIR. 'image/login_input_bg.png';?>');}
.submit_bg_forie6{position:absolute;width:80px;height:42px;overflow:hidden;}
.submit_bg_forie6 .inputbg{position:absolute;width:80px;height:42px;filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='<?php echo $_TEMPLATESDIR. 'image/login_submit.png';?>');}
</style>
<![endif]-->
...
function input_key_up(e,obj,type){
	...
	if (obj.value==''){
		var v = {'username':'用户名','password':'密码','checkcode':'验证码'};
...
function clear_checkcode(e,obj,type){
	var tobj = document.getElementById('checkcode');
		tobj.value='';
	    input_key_up({'keyCode':13},document.getElementById('checkcode'),'checkcode');
		...
		<table border="0" align="center" height="100%" width="500">
			...
			<tr>
				<td height="200" align="left" valign="top">
					<div style="position: absolute; text-align: left; width: 400px; height: 190px; margin-left: 90px; background: url(<?php echo $_TEMPLATESDIR. 'image/admin.png';?>) no-repeat left top; _background: none; _filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src='<?php echo $_TEMPLATESDIR. 'image/admin.png';?>' );">
						<div style="position: absolute; margin: 31px 0 0 231px;">
							<table border="0" cellpadding="0" cellspacing="0" width="30"
								height="45">
								<tr>
									<td align="left" valign="bottom"><img id="image1"
										style="width: 38px; height: 44px;"
										src="<?php echo $_TEMPLATESDIR. 'image/admin_p2.png';?>" />
									</td>
								</tr>
							</table>
						</div>
						<div style="position: absolute; margin: 28px 0 0 232px;">
							<table style="width: 60px; height: 50px;" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td align="left" valign="bottom"><img id="image2"
										style="width: 60px; height: 48px;"
										src="<?php echo $_TEMPLATESDIR. 'image/admin_p1.png';?>" />
									</td>
								</tr>
							</table>
						</div>
						<div style="position: absolute; width: 400px; height: 190px; background: url(<?php echo $_TEMPLATESDIR. 'image/login_spacer.gif';?>);"></div>
					</div>
<script type="text/javascript">
(function(){
	...
	if (is_ie && ie<7){
		...
		var src1 = obj1.src;
		var src2 = obj2.src;
		obj1.src = obj2.src = '<?php echo $_TEMPLATESDIR. 'image/login_spacer.gif';?>';
	...
	setInterval(run,80);
			...
			<tr>
				<td height="200" valign="top">
					<div id="login_form">
						<form action="?action=checklogin" method="post" id="myform" autocomplete="off"	onsubmit="return check_submit(this);">
							...
							<div class="input_div" style="margin-left: -60px;">
								...
								<div style="padding: 28px;">
									...
									<input tabindex="1" type="text" size="30" name="username"
										id="username" value="" class="input"
										onkeydown="input_key_down(event);"
										onkeyup="input_key_up(event,this,'username');"
							...
							<div class="input_div" style="margin-left: 193px;">
								...
								<div style="padding: 28px;">
									...
									<input tabindex="2" type="password"
										style="font-family: Tahoma, Simsun, Helvetica, sans-serif;"
										name="password" size="30" id="password" class="input"
										onkeydown="input_key_down(event);"
										onkeyup="input_key_up(event,this,'password');"
										onfocus="heightlight(this.parentNode.parentNode)"
										onblur="offlight(this.parentNode.parentNode)" />
								</div>
							</div>
							...
							<div class="submit_bg" id="submit_bg" style="margin-left: 475px;">
								...
								<div class="submit_text" onmousedown="show_hover()"
									onmouseup="show_up()"
									onclick="var obj=document.getElementById('myform');if (check_submit(obj))obj.submit();">登&nbsp;录</div>
							</div>
							<div style="clear:both;height:90px;"></div>
							<?php if(isset($_SESSION['msg'])){?><div style="position:absolute;margin-left:-30px;margin-top:55px;font-size:13px;color:#ff0;"><?php echo $_SESSION['msg'];?></div><?php }?>
...
<?php if(isset($_SESSION['msg'])){?>shake(p,20);<?php }?>
</script>
...
if (Detector.webgl){
    ...
    if ( !disabled_cloud ){
        ...
        var img = new Image('<?php echo $_TEMPLATESDIR. 'image/cloud10.png';?>');
        setTimeout('img=null;',2000);
    document.write('<script type="text/javascript" src="<?php echo $_TEMPLATESDIR.'js/ThreeWebGL.js';?>"></'+'script><script type="text/javascript" src="<?php echo $_TEMPLATESDIR.'js/ThreeExtras.js';?>"></'+'script>');
}else{
...
function init() {
    ...
    var texture = THREE.ImageUtils.loadTexture('<?php echo $_TEMPLATESDIR. 'image/cloud10.png';?>');
